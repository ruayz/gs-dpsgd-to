...
from utils import *
from evaluators.metrics import accuracy, accuracy_per_group
...
class BaseTrainer:
    ...
    def _train_epoch(self, cosine_sim_per_epoch, expected_loss, train_loder, param_for_step=None):
        ...
        for _batch_idx, (data, target, group) in enumerate(tqdm(train_loder)):
            ...
            elif self.method in ["dpsgd", "dpsgd-post", "dpsgd-global", "dpsgd-global-adapt"]:
                grad_norms, _, sum_grad_vec_batch, sum_clip_grad_vec_batch = self.get_sum_grad_batch_from_vec(
            ...
            if (self.evaluate_angles or self.evaluate_hessian) and (
                ...
                elif self.method in ["dpsgd", "dpsgd-post", "dpsgd-f", "dpsgd-global", "dpsgd-global-adapt"]:
    ...
    def _train_separate(self, net, optimizer, cosine_sim_per_epoch, expected_loss, train_loder, number, param_for_step=None):
        net.train()
        # methods: separate
        criterion = torch.nn.BCEWithLogitsLoss()  # sigmoid()+loss
        #criterion = torch.nn.CrossEntropyLoss()
        losses = []
        all_grad_norms = []
        #all_grad_norms = [[] for _ in range(self.num_groups)]
        ...
        for _batch_idx, (data, target, group) in enumerate(tqdm(train_loder)):
            ...
            #losses_per_group = self.get_losses_per_group(criterion, data, target, group, losses_per_group)
            loss.backward()
            per_sample_grads = self.flatten_all_layer_params(net)

            # get sum of grads over groups over current batch
            grad_norms, clip_grad_norms, sum_grad_vec_batch, sum_clip_grad_vec_batch = self.get_sum_grad_batch_from_vec(
                per_sample_grads, group, clipping_bound=self.max_grad_norm)
            ...
            for i in range(self.num_groups):
                if len(grad_norms[i]) != 0:
                    all_grad_norms = all_grad_norms + grad_norms[i]
                    group_max_grads = max(group_max_grads, max(grad_norms[i]))
                    #g_B_k_norms.append(torch.linalg.norm(torch.mean(torch.tensor(all_grad_norms))).item())

            optimizer.step()
            losses.append(loss.item())
        if self.method != "regular":
            epsilon = self.privacy_engine[number].get_epsilon(delta=self.delta)
            print(f"(ε = {epsilon:.2f}, δ = {self.delta})")
            # privacy_dict = {"epsilon": epsilon, "delta": self.delta}
            # self.writer.record_dict(f"Privacy{group}", privacy_dict, step=0, save=True, print_results=False)
        group_ave_grad_norms = np.mean(all_grad_norms)
        #group_ave_grad_norms = [np.mean(all_grad_norms[i]) for i in range(self.num_groups)]
        print(f"group_ave_grad_norms:{group_ave_grad_norms}")
        group_norm_grad_ave = np.mean(g_B_k_norms)

        return net.state_dict(), group_ave_grad_norms, group_max_grads, group_norm_grad_ave, copy.deepcopy(losses), epsilon
    ...
    def train(self, write_checkpoint=True):
        ...
        while self.epoch < self.max_epochs:
            ...
            if self.epoch == 0:
                ...
                group_acc_per_epochs = accuracy_per_group(self.model, self.train_loader,
                                                          num_groups=2, method=self.method)
                #print(f"group_acc_per_epochs: {group_acc_per_epochs}")
                group_acc_epochs.append([-1, acc_per_epoch] + list(group_acc_per_epochs))

            if self.method == "separate" or self.method == "dpsgd-thresh":
                if self.method == "separate":
                    if self.parameter_norm_bound == None:
                        pass
                    else:
                        # 对最后一层权重进行 L2 范数裁剪
                        with torch.no_grad():
                            last_layer_weights = self.model.net[-1].weight.data
                            self.model.net[-1].weight.data = clip_weights(last_layer_weights, self.parameter_norm_bound)
                ...
                #for group in range(1):
                    ...
                    if self.method == "separate":
                        if self.epoch >= 0.9*self.max_epochs:
                            optimizer_separate = torch.optim.SGD(model_copy.parameters(), lr=0.005)
                        else:
                            optimizer_separate = torch.optim.Adam(model_copy.parameters(), lr=self.lr)
                    elif self.method == "dpsgd-thresh":
                        optimizer_separate = torch.optim.Adam(model_copy.parameters(), lr=self.lr)
                    # optimizer = torch.optim.SGD(self.model.parameters(), lr=0.005)
                    # model_copy, optimizer_separate, train_loader = self.privacy_engine[group].make_private(
                    #     module=model_copy,
                    #     optimizer=optimizer_separate,
                    #     data_loader=self.train_loader[group],
                    #     noise_multiplier=self.noise_multiplier[group],
                    #     # clipping="adaptive",
                    #     max_grad_norm=self.max_grad_norm  # C
                    # )
                    #optimizer_separate = torch.optim.SGD(model_copy.parameters(), lr=self.lr)
                    model_copy, optimizer_separate, train_loader = self.privacy_engine[group].make_private_with_epsilon(
                        module=model_copy,
                        optimizer=optimizer_separate,
                        data_loader=self.train_loader[group],
                        #data_loader=self.train_loader[2],
                        target_epsilon=self.target_epsilon,
                        target_delta=self.delta,
                        epochs=self.max_epochs,
                        max_grad_norm=self.max_grad_norm  # C
                    )
                    w_, avg_grad_norms_, max_grads_, norm_avg_grad_, loss_, epsilon = self._train_separate(
                        model_copy,
                        optimizer_separate,
                        cos_sim_per_epoch,
                        expected_loss,
                    ...
                    avg_grad_norms.append(avg_grad_norms_)
                    max_grads.append(max_grads_)
                    norm_avg_grad.append(norm_avg_grad_)
                    losses.append(np.squeeze(loss_))
                    group_losses.append(np.mean(loss_))
                    w_locals.append(copy.deepcopy(w_))
                    epsilons[group] = epsilon

                losses = np.concatenate(losses)
                w_glob = FedAvg(w_locals)
                # ratio = np.square(group_losses[0] / group_losses[1])
                # w_glob = FedWeightAvg(w_locals, [ratio/(ratio+1), 1/(ratio+1)])
                privacy_dict = {"epsilon": np.max(epsilons), "delta": self.delta}
                self.writer.record_dict("Privacy", privacy_dict, step=0, save=True, print_results=False)

                # 修改键名
                state_dict = {k.replace("_module.", ""): v for k, v in w_glob.items()}
                # 加载修改后的 state_dict
                self.model.load_state_dict(state_dict)
                #self.model.load_state_dict(w_glob)

                # compute acc
                acc_per_epoch = accuracy(self.model, self.train_loader[2], method=self.method)
                group_acc_per_epochs = accuracy_per_group(self.model, self.train_loader[2],
                                                          num_groups=2, method=self.method)
            ...
            group_acc_epochs.append([self.epoch, acc_per_epoch] + list(group_acc_per_epochs))
        ...
        #print(self.model.state_dict())  # new
        K = self.num_groups
        #K = 1
...
class DpsgdSTrainer(BaseTrainer):
    """Class for DPSGD separate training"""

    # given norm of gradient, computes S such that clipped gradient = S * gradient
    def clipping_scale_fn(self, grad_norm, idx, clipping_bound):
        return min(1, clipping_bound / (grad_norm + 1e-10))
    ...
    def __init__(
        ...
        self.delta = delta
        # new parameters
        self.parameter_norm_bound = parameter_norm_bound
        #self.noise_multiplier = noise_multiplier
        self.target_epsilon = target_epsilon
        self.max_grad_norm = max_grad_norm


class DpsgdTTrainer(BaseTrainer):
    """Class for DPSGD thresh training"""
