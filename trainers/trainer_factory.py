from .trainer import RegularTrainer, DpsgdTrainer, DpsgdSTrainer, DpsgdTTrainer, DpsgdFTrainer, DpsgdGlobalTrainer, DpsgdGlobalAdaptiveTrainer
...
def create_trainer(
    ...
    if config["method"] == "regular":
        ...
        trainer = DpsgdSTrainer(
            ...
            device,
            delta=config["delta"],
            parameter_norm_bound=config["parameter_norm_bound"],
            #noise_multiplier=config["noise_multiplier"],
            target_epsilon=config["target_epsilon"],
            max_grad_norm=config["l2_norm_clip"],
            **kwargs
        )
    elif config["method"] == "dpsgd-thresh":
        trainer = DpsgdTTrainer(
            model,
            optimizer,
            privacy_engine,
            train_loader,
